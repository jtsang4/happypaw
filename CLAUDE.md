...
- **输入**：飞书 / Telegram / QQ / 微信 / Web 界面消息（每个用户可独立配置 IM 通道）
- **执行**：Docker 容器或宿主机进程中运行固定版本 Codex，每个用户拥有独立主容器
- **输出**：飞书富文本卡片 / Telegram HTML / QQ / 微信 纯文本 / Web 实时流式推送
...
| `src/routes/config.ts` | Codex 默认配置、系统设置与 per-user IM 通道配置（`/api/config/codex`、`/api/config/user-im/feishu`、`/api/config/user-im/telegram`、`/api/config/user-im/qq`、`/api/config/user-im/wechat`） |
...
| `src/im-manager.ts` | IM 连接池管理器（`IMConnectionManager`）：per-user 飞书/Telegram/QQ/微信 连接管理、热重连、批量断开 |
...
| `/setup/providers` | `SetupProvidersPage` — Codex 默认配置与可选飞书接入 | 登录后 |
...
- **会话循环**：处理一轮 Codex turn → 等待 IPC 消息 → 再次处理 → 直到 `_close` sentinel
- **MCP Server**：12 个工具（`send_message`、`schedule_task`、`list/pause/resume/cancel_task`、`register_group`、`install_skill`、`uninstall_skill`、`memory_append`、`memory_search`、`memory_get`），通过 HappyPaw 内置 MCP bridge 注册
- **Hooks**：PreCompact 钩子在上下文压缩前归档对话到 `conversations/` 目录
- **敏感数据过滤**：StreamEvent 中的 `toolInputSummary` 会过滤 `ANTHROPIC_API_KEY` 等环境变量名
- **预定义 SubAgent**：`agent-definitions.ts` 定义 `code-reviewer`（代码审查）和 `web-researcher`（网页研究）两个 SubAgent，作为运行时内置 Agent 定义注册

**Agent Runner 模块结构**（`container/agent-runner/src/`）：

| 文件 | 职责 |
|------|------|
| `index.ts` | 主入口：stdin 读取、会话循环、Codex runtime 调用、IPC 轮询 |
...
| `mcp-tools.ts` | MCP 工具定义：12 个工具的 bridge 实现，负责 IPC 文件通信 |
...
| 模式 | 行为 | 适用对象 | 前置依赖 |
|------|------|---------|---------|
| `host` | Agent 作为宿主机进程运行，通过 HappyPaw 管理的固定 Codex 二进制直接访问宿主机文件系统 | admin 主容器（`folder=main`） | Node.js；首次启动自动下载固定 Codex |
...
宿主机模式通过 `node container/agent-runner/dist/index.js` 启动 agent-runner 进程，agent-runner 内部调用 Codex App Server；HappyPaw 会在首次宿主机启动时下载并缓存固定版本 Codex，无需全局安装。
...
- 安装 `agent-browser`，并将固定版本 Codex 二进制内置到镜像
- entrypoint 会把挂载的 Skills 链接到 `/home/node/.codex/skills`
...
```
飞书/Telegram/QQ/微信/Web 消息 → storeMessageDirect(db) + broadcastNewMessage(ws)
     ...
     → 流式输出 → onOutput 回调
         → imManager.sendFeishuMessage()/sendTelegramMessage()/sendQQMessage()/sendWeChatMessage() + broadcastToWebClients() + db.storeMessageDirect()
```

### 3.2 流式显示管道

```
Codex App Server → 流式事件 (text_delta, tool_use_start, ...)
...
| Codex 运行目录 `data/sessions/{folder}/.codex/` | `/home/node/.codex` | 读写 | 读写（仅自己） |
| IPC 通道 `data/ipc/{folder}/` | `/workspace/ipc` | 读写 | 读写（仅自己） |
| 项目级 Skills `container/skills/` | `/workspace/project-skills` | 只读 | 只读 |
| 用户级 Skills `~/.codex/skills/` | `/workspace/user-skills` | 只读 | admin 创建的会话可读 |
...
- 每个用户可独立配置飞书、Telegram、QQ 和微信连接（存储在 `data/config/user-im/{userId}/feishu.json`、`telegram.json`、`qq.json`、`wechat.json`）
- `feishu.ts`、`telegram.ts`、`qq.ts`、`wechat.ts` 均为工厂模式（`createFeishuConnection()`、`createTelegramConnection()`、`createQQConnection()`、`createWeChatConnection()`），返回无状态的连接实例
...
| 权限 | 说明 |
|------|------|
| `manage_system_config` | 管理系统配置（Codex 默认接入 / IM / 系统参数） |
...
| IM 通道 | 独立的飞书/Telegram/QQ/微信连接 | 独立的飞书/Telegram/QQ/微信连接 |
| 全局记忆写入 | 可读写 | 只读 |
| 项目根目录挂载 | 读写 | 不可访问 |
...
| `sessions` | `(group_folder, agent_id)` | 会话 ID 映射（Codex thread/session 持久化，支持 Sub-Agent 独立会话） |
...
data/
  ...
  sessions/{folder}/.codex/                # Codex 线程 / 运行时持久化（隔离）
  ...
  memory/{folder}/                         # 日期记忆
  config/                                  # 加密配置文件
  config/codex-provider.json               # Codex 默认配置
...
### 配置
- `GET|PUT /api/config/codex` · `PUT /api/config/codex/secrets`
...
首次启动时，`GET /api/auth/status` 返回 `initialized: false`（无任何用户）。前端 `AuthGuard` 检测到未初始化状态后重定向到 `/setup`，引导创建管理员账号（自定义用户名 + 密码，调用 `POST /api/auth/setup`）。创建后自动登录并跳转到 `/setup/providers` 完成 Codex 默认配置和可选飞书接入。

新用户注册后跳转到 `/setup/channels` 引导配置个人 IM 通道（飞书/Telegram/QQ/微信），可跳过直接使用 Web 聊天。

不存在默认账号。`POST /api/auth/setup` 仅在用户表为空时可用。

### 8.2 IM 自动注册

未注册的飞书/Telegram/QQ/微信会话首次发消息时，通过 `onNewChat` 回调自动注册到该用户的主容器（`folder='home-{userId}'`，admin 则为 `folder='main'`）。支持多个 IM 群组映射到同一个 folder。QQ 通道需先通过配对码绑定（`/pair <code>`）。
...
每个会话拥有独立的 `groups/{folder}` 工作目录、`data/sessions/{folder}/.codex` 运行目录、`data/ipc/{folder}` IPC 命名空间。非主会话只能发消息给自己所在的群组。
...
主容器在 Web 与 IM 共用历史（通过 `normalizeHomeJid` 映射飞书/Telegram/QQ/微信 JID → `web:{folder}`）。IM 来源的消息回复到对应 IM 渠道，Web 来源的消息仅在 Web 展示。
...
飞书/Telegram/QQ/微信 中以 `/` 开头的消息会被拦截为斜杠命令（未知命令继续作为普通消息处理）。命令在主服务进程的 `handleCommand()` 中分发，纯函数逻辑在 `im-command-utils.ts` 中（便于单测）。
...
| `/where` | - | 查看当前绑定位置与回复策略 |
| `/bind <workspace>` | - | 将当前 IM 对话绑定到指定工作区或 Agent |
| `/unbind` | - | 解绑回默认工作区 |
| `/new <名称>` | - | 创建新工作区并绑定当前 IM 对话 |
| `/recall` | `/rc` | 已移除；当前会返回明确的 Codex-only 提示文案 |
| `/clear` | - | 清除当前对话的会话上下文 |
| `/require_mention` | - | 切换群聊响应模式：`/require_mention true`（需要 @机器人）或 `/require_mention false`（全量响应） |

`/where`、`/bind`、`/unbind`、`/new` 用于管理 IM 与工作区绑定；`/recall` 不再调用任何旧版 CLI。
...
2. 用户级：添加到 `~/.codex/skills/`（自动挂载到所有容器）
