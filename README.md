<p align="center">
  <img src="web/public/icons/logo-1024.png" alt="HappyPaw Logo" width="120" />
</p>

<h1 align="center">HappyPaw</h1>

<p align="center">
  自托管的多用户本地 AI Agent 系统 — Powered by Codex.
</p>
...
<p align="center">
  <a href="#happypaw-是什么">介绍</a> · <a href="#核心能力">核心能力</a> · <a href="#快速开始">快速开始</a> · <a href="#技术架构">技术架构</a> · <a href="#贡献">贡献</a>
</p>
...
HappyPaw 是一个自托管的多用户 AI Agent 系统。它通过 HappyPaw 管理的固定版本 Codex 运行时（容器内置、宿主机首启自动下载并缓存）向 Web、飞书、Telegram、QQ、微信等入口提供统一的 Agent 能力，支持文件读写、终端操作、浏览器自动化、多轮推理及 MCP 工具生态。

核心设计原则：**只暴露一个支持的运行时：Codex**。HappyPaw 负责固定版本分发、`.codex` 运行目录准备、MCP 桥接与流式事件翻译；运维只需配置 Codex / OpenAI 兼容网关与可选消息通道，不需要手动安装全局 `codex`，也不再需要任何旧版运行时接入流程。

### 关键特性

- **固定 Codex 运行时** — 容器内置 pinned Codex 二进制，宿主机模式首启自动下载并复用缓存；所有支持路径都不依赖环境中的全局 `codex`
...
> HappyPaw 不是从零开始，而是以 [HappyClaw](https://github.com/riba2534/happyclaw) 为底座持续分化演进而来：在继承其产品形态与部分工程基础的前提下，本项目对底层运行逻辑进行了系统性重构，将原有运行时体系完整替换为 Codex，并围绕自托管、多用户隔离、Web / IM 统一接入等方向继续扩展。感谢 HappyClaw 项目提供的坚实起点。
...
| **微信** | iLink Bot API | 纯文本 | 二维码登录、1:1 绑定、图片消息自动下载到工作区 |
| **Web** | WebSocket 实时通信 | 流式 Markdown | 图片粘贴/拖拽上传、虚拟滚动 |

每个用户可独立配置自己的 IM 通道（飞书应用凭据、Telegram Bot Token、QQ Bot 凭据、微信登录态），互不干扰。消息统一路由：飞书来源回飞书，Telegram 来源回 Telegram，QQ 来源回 QQ，微信来源回微信，Web 来源回 Web。


### Agent 执行引擎

HappyPaw 通过统一的 runner 适配层启动 Codex App Server，并保持既有的 `ContainerInput -> ContainerOutput -> StreamEvent` 产品边界。

- **Per-user 主工作区** — 每个用户拥有一个固定的主工作区（admin 使用宿主机模式，member 使用容器模式），IM 消息路由到各自的主工作区
- **宿主机模式** — Agent 直接在宿主机运行，HappyPaw 首次启动会下载并缓存固定版本 Codex（admin 主工作区默认模式）
- **容器模式** — Docker 隔离执行，非 root 用户，镜像内置固定版本 Codex 与 40+ 工具（member 主工作区默认模式）
...
- **用户级 Skills** — 放在 `~/.codex/skills/`，所有容器自动挂载
...
- **Codex / OpenAI 兼容 API Key** — 系统初始化至少需要一个 API Key，可在 Web 向导中配置，支持默认官方网关或自定义 Base URL
...
- 微信账号登录 — 仅微信集成需要，可在设置页通过二维码绑定

> 无需手动安装 Codex CLI——`make start` / `make build` 会自动准备 HappyPaw 固定版本的 Codex 二进制；容器镜像也会内置相同版本的运行时。

### 安装启动

```bash
# 1. 克隆仓库
git clone https://github.com/jtsang4/happypaw.git
cd happypaw
...
2. **配置 Codex 默认接入** — 填入 API Key；Base URL / Model 可按需覆盖，留空则使用默认值
3. **配置 IM 通道**（可选）— 可在设置页或注册后的 `/setup/channels` 中继续配置飞书、Telegram、QQ、微信
4. **开始对话** — 在 Web 聊天页面直接发送消息

> 所有配置通过 Web 界面完成，不依赖任何配置文件。Codex 密钥与 IM 凭据均以 AES-256-GCM 加密存储。
...
4. 发布应用版本并等待审批通过
5. 在 HappyPaw Web 界面的「设置 → IM 通道 → 飞书」中填入 App ID 和 App Secret
...
2. 记录返回的 Bot Token
3. 在 HappyPaw Web 界面的「设置 → IM 通道 → Telegram」中填入 Bot Token
...
4. 在 HappyPaw Web 界面的「设置 → IM 通道 → QQ」中填入 App ID 和 App Secret
...
1. 打开 HappyPaw Web 界面的「设置 → IM 通道 → 微信」
2. 点击获取二维码，前端会调用 `POST /api/config/user-im/wechat/qrcode` 获取登录二维码
3. 使用微信扫码并确认登录；系统会轮询 `GET /api/config/user-im/wechat/qrcode-status?qrcode=...` 完成绑定并自动保存登录态
4. 绑定成功后即可通过微信 1:1 对话使用 HappyPaw；如需断开，可在设置页执行断开登录（`POST /api/config/user-im/wechat/disconnect`）

> 微信通道使用 iLink Bot API，当前为 1:1 绑定模式，无需像 QQ 那样额外配对。图片会自动下载到工作区，`/list`、`/bind`、`/clear` 等斜杠命令与其他 IM 通道一致；`/recall` 已移除，只会返回 Codex-only 提示文案。

### IM 斜杠命令

飞书/Telegram/QQ/微信 中以 `/` 开头的消息会被拦截为斜杠命令（未知命令继续作为普通消息处理）：
...
| `/recall` | `/rc` | 已移除；当前只会返回 Codex-only 提示文案，不再执行总结 |
| `/clear` | - | 清除当前对话的会话上下文 |
...
| 模式 | 说明 | 适用对象 | 前置要求 |
|------|------|---------|---------|
| **宿主机模式** | Agent 直接在宿主机运行，HappyPaw 首次启动自动下载并缓存固定版本 Codex | admin 主工作区（`folder=main`） | Node.js；无需手动安装 `codex` |
| **容器模式** | Agent 在 Docker 容器中隔离运行，镜像已内置固定版本 Codex 与 40+ 工具 | member 主工作区（`folder=home-{userId}`） | Docker Desktop + 构建镜像 |

admin 主工作区默认使用宿主机模式，member 注册后自动创建容器模式的主工作区。执行模式只影响宿主机 / 容器承载方式，AI 运行时始终固定为 Codex。
...
| 类别 | 工具 |
|------|------|
| AI / Agent | Pinned Codex Binary、Codex App Server、MCP Bridge |
...
flowchart TD
    ...
    subgraph 执行层
        Host["宿主机进程<br/>(Managed Codex)"]
        Container["Docker 容器<br/>(agent-runner + bundled Codex)"]
    end

    subgraph Agent["Codex 运行时"]
        SDK["Codex App Server<br/>(thread / stream)"]
...
**数据流**：消息从接入层进入主进程，经去重和路由后分发到并发队列。队列启动宿主机进程或 Docker 容器，内部的 agent-runner 调用 Codex App Server，并将其事件翻译为 HappyPaw 既有的流式协议。流式事件（思考、文本、工具调用等 12 种类型）通过 stdout 标记协议传回主进程，再经 WebSocket 广播到 Web 客户端或通过 IM API 回复到飞书、Telegram、QQ、微信。MCP Server 通过基于文件的 IPC 通道提供 12 个工具，实现 Agent 与主进程的双向通信。
...
| **Agent** | Codex App Server · Pinned Codex Binary · MCP Bridge · IPC 文件通道 |
...
以下为可选覆盖项。推荐使用 Web 设置向导配置 Codex / OpenAI 兼容网关与 IM 凭据（加密存储）。

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `WEB_PORT` | `3000` | Web 服务端口 |
| `ASSISTANT_NAME` | `HappyPaw` | 助手显示名称 |
| `OPENAI_API_KEY` | - | Codex / OpenAI 兼容网关 API Key（作为 Web 配置缺省值） |
| `OPENAI_BASE_URL` | - | 自定义 Codex / OpenAI 兼容 Base URL |
| `OPENAI_MODEL` | - | 默认模型名 |
| `CONTAINER_IMAGE` | `happypaw-agent:latest` | Agent 容器镜像 |
...
> 提交或发版前，建议至少运行 `make typecheck` 与 `make build`；如改动了特定能力，再执行 `tests/*.mjs` 中对应的定向回归脚本。
...
[![Star History Chart](https://api.star-history.com/svg?repos=jtsang4/happypaw&type=date&legend=top-left)](https://www.star-history.com/#jtsang4/happypaw&type=date&legend=top-left)
