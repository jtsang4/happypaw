  ...
  return (
    <div ref={containerRef} className="h-full flex flex-col bg-background">
          ...
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            {!isWaiting && group.execution_mode && (
              <>
                <span className="text-muted-foreground/40">·</span>
                <span
                  className={`inline-flex items-center px-1 py-px rounded text-[10px] font-medium ${group.execution_mode === 'host' ? 'bg-amber-100 text-amber-700' : 'bg-sky-100 text-sky-700'}`}
                >
      ...
      <ConfirmDialog
        ...
        message={
          resetAgentId
            ? '将清除该子对话的 Codex 会话上下文，下次发送消息时将开始全新会话。聊天记录不受影响。'
            : '将清除 Codex 会话上下文并停止运行中的工作区进程，下次发送消息时将开始全新会话。聊天记录不受影响。'
        }
        confirmText="清除"
