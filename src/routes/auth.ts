...
function buildSessionCookie(
  ...
  return `${name}=${value}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${maxAgeSeconds}${secureSuffix}`;
}

function appendSetCookieHeaders(headers: Headers, values: string[]): void {
...
export function setSessionCookieHeaders(c: any, token: string): Headers {
  ...
  const activeCookieName = getSessionCookieName(secure);
  const headers = new Headers({ 'Content-Type': 'application/json' });
  const cookies = [
    buildSessionCookie(activeCookieName, token, secure, 30 * 24 * 60 * 60),
  ];
  for (const [name, isSecure] of COOKIE_SECURE_FLAGS.entries()) {
    if (name === activeCookieName) continue;
    cookies.push(buildSessionCookie(name, '', isSecure, 0));
  }
...
export function clearSessionCookieHeaders(): Headers {
  ...
  appendSetCookieHeaders(
    headers,
    [...COOKIE_SECURE_FLAGS.entries()].map(([name, isSecure]) =>
      buildSessionCookie(name, '', isSecure, 0),
    ),
  );
  return headers;
}
...
authRoutes.post('/setup', async (c) => {
  ...
  return new Response(
    ...
    {
      status: 201,
      headers: setSessionCookieHeaders(c, token),
    },
  );
});
...
authRoutes.put('/password', authMiddleware, async (c) => {
  ...
  return new Response(
    ...
    {
      status: 200,
      headers: setSessionCookieHeaders(c, newToken),
    },
  );
});
