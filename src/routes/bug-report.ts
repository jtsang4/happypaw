...
bugReportRoutes.post('/submit', authMiddleware, async (c) => {
  ...
  const fullBody = `${body}\n\n---\n> Submitted via HappyPaw by ${user.display_name || user.username}`;
  ...
  const url = `https://github.com/jtsang4/happypaw/issues/new?title=${encodeURIComponent(title)}&body=${encodeURIComponent(truncatedBody)}`;
