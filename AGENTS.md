...
- Codex App Server 协议产物只以 `generated/codex-app-server-protocol/` 为单一真相源；该目录由仓库固定版本的 `codex app-server` 命令生成
- `container/agent-runner/src/generated/codex-app-server-protocol/` 只是给 agent-runner 使用的镜像副本，由 `make sync-types` 从根目录产物同步；不要手改镜像，也不要把它当作新的真相源
...
补充约定：如果需要升级或刷新 Codex App Server 协议产物，应先执行 `npm run generate:codex-protocol` 更新根目录 `generated/codex-app-server-protocol/`，再执行 `make sync-types` 同步 `container/agent-runner/src/generated/codex-app-server-protocol/` 镜像，最后运行 `npm run test:codex-protocol` 校验两者一致。
